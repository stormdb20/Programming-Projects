...
class Hotel:
#Name attribute for the hotel that is passed when hotel is instaniated and create parallel lists for dog names and hotel names
...
#compare to make sure it is a dog object then add dog name and dog object to the list with messages for both outcomes
...
#method to check out that iterates through the kennel names list deleting values from both lists if name equals name on kennel list 
    def check_out(self, name):
        for i in range(0, len(self.kennel_names)):
            if name == self.kennel_names[i]:
                ...
                #print message and return dog object
...
def test_code():
    ...
    lonnie = Cat('Lonnie')
    #create hotel object passing a name and use hotel method on the various dogs and cat
    hotel = Hotel('Paws')
    hotel.check_in(cody)
    hotel.check_in(jackson)
    hotel.check_in(rody)
    hotel.check_in(guy)
    hotel.check_in(lonnie)
    #use dog var for check out method passing the [insert name here].name
    dog = hotel.check_out(cody.name)
    print('Checked out', dog.name, 'who is', dog.age, 'and weighs', dog.weight, 'lbs')
    dog = hotel.check_out(jackson.name)
    print('Checked out', dog.name, 'who is', dog.age, 'and weighs', dog.weight, 'lbs')
    dog = hotel.check_out(rody.name)
    print('Checked out', dog.name, 'who is', dog.age, 'and weighs', dog.weight, 'lbs')
    dog = hotel.check_out(guy.name)
    print('Checked out', dog.name, 'who is', dog.age, 'and weighs', dog.weight, 'lbs')
    dog = hotel.check_out(tommy.name)
